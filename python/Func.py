...
def GetPoster_single(tt):
    ...
    res= poster_list[poster_list['imdbID']==tt]['poster_path'].values[0]
    del poster_list
    return res

def GetPoster(ll):
    poster_list= pd.read_csv('materials/fastlink_poster.csv')
    res= poster_list[poster_list['imdbID'].isin(ll)]['poster_path'].to_list()
    del poster_list
    return res
...
def get_movie_id(name):
    ...
    try:
        res =  df[df.Title == name.strip()]['imdbID'].values[0]
    except:
        res=  df[df.Title.apply(lambda x: x.lower()) == cloest_name(name)]['imdbID'].values[0]
    del df
    return res
...
def reco_knn(movie_list):
        ...
        res = tmp[~tmp['imdbID'].isin(movie_list)]['imdbID'].to_list()[:20]
        return res
